...
async function sendMessage(target, text, isConversation = true) {
  if (!OA_TOKEN) return console.error('Không có OA_TOKEN, không thể gửi tin nhắn.');
...
app.post('/webhook', async (req, res) => {
  ...
  if (!state.counting) return; // bỏ qua nếu không đếm
...
app.listen(port, () => console.log(`Zalo image-user counter bot chạy port ${port}`));
